...
仓库同时保留最初的浏览器阅读器（`public/`），适合体验与旧书库迁移：

```bash
python server.py   # 或双击 start.bat（macOS / Linux 用 ./start.sh）
```

浏览器自动打开 `http://127.0.0.1:8765`，手机与电脑同一 Wi-Fi 时访问启动时打印的局域网地址。旧书库可在 Windows 客户端「从浏览器迁移」中预检后整体迁入。

> 没有 API Key 也能体验：`python tools/mock_llm.py` 启动本地演示模型，设置里填 `http://127.0.0.1:8799/v1`，Key 随意，模型填 `mock-reader-1`。
...
- `skills/legacy/` 是浏览器阅读器的旧五技能留档（浏览器端正式来源）；
...
```
paper-30min/
├── server.py            # 浏览器阅读器本地服务器
├── start.bat / start.sh # 浏览器阅读器一键启动
├── public/              # 浏览器阅读器前端（纯原生 JS，无需构建）
