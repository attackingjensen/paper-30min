...
stage: map-l2
---
<!-- Token 预算（#38 实测）：输入 ≈ 全文文本层（典型论文 2–6 万 token；全文超 ~20 万 token 时由调用方按节分片多次调用本提示词，再确定性合并）+ 关注点表数百 token；输出 ≈ 每节 1500 字量级 × 节数。任一调用装配后超 983,616 输入 token 由调用方报错拒绝，不截断。 -->
