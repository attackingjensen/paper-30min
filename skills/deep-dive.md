...
stage: deep-dive
---
<!-- Token 预算：常驻 = L1 地图 + 全部 L2（每节 1500 字量级）+ 当前节原文全送；当前节页图 ±1 页与本节关键图表裁切图（至多 4 张）由调用方随消息附图（页图约 1902 token/页、裁切图约 1024 token/张）。工具循环最多 12 步、一轮至多 3 个工具调用块，工具结果只回窗口或指针。 -->
