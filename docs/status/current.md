...
- [#92](https://github.com/attackingjensen/paper-30min/issues/92) 已修复关闭：进度域在已建图时以 L2 产物为权威（票内选方向 1，未做「重新建图」入口）。存量论文（建图早于 #87）的记录 `parts` 本身仍未对齐——内容投影（原文 tab / 旧结果留存）沿用 pdf.js 形状，readMarks 孤儿不迁移（见上条），触发对齐仍需重新导入或未来的重跑入口。
