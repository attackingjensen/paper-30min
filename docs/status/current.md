...
- 根目录 `npm test`：120 项通过（含 #60 回归夹具清单良构 3 项）。
- `cd app && node --test`：37 项通过。
- `cd app/src-tauri && cargo test`：159 项通过（含 `pdfparse_contract` 8 项、`pdfmap` 映射层内联 25 项与实测 fixture 6 篇、`pdfassets_contract` 5 项、`pdfparse_regression` 13 项——默认跑清单良构，全链 assert 模式本机实测 13/13 全绿）。
