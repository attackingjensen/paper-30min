...
- 根目录 `npm test`：123 项通过（含 #60 回归夹具清单良构 3 项、#61 前端 applyResplit/导出携带 3 项）。
- `cd app && node --test`：40 项通过。
