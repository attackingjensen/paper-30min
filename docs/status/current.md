...
- `npm test` 是统一测试入口，串起 `node --test`（自动发现 `tests/*.test.mjs`，当前 25 个测试）与 `tools/check_markdown.mjs`。`tools/check_parser.mjs` 没有断言、只输出 JSON 供人工查看，因此不在门禁内。
...
- 同日与作者完成「论文记录生命周期」grilling 设计并全部拍板：`papers.js` deep module 统一拥有记录生命周期，设计由 [ADR-0004](../adr/0004-paper-lifecycle-module.md) 接受，四步实施计划经 Issue #9 跟踪。第 1 步已完成：`papers.js` 骨架（存储缝、两处创建、精读部分投影、阅读进度）、`app.js` 创建点切换、`db.js` 吸收删除，25 个测试全绿；尚未做浏览器端验证。
...
1. 继续 Issue #9 第 2 步：把各功能写入点逐一迁移为 `papers.js` 带规则的写入函数（`saveAnalysis`、`saveSectionSource`、`saveRecallCard`、`saveTranslation`、`appendChatMessage`、`saveOrganize`、`attachPdf`、`setNumPages`），完成后移除过渡用的 `saveRecord`；每步保持 `npm test` 全绿。
