...
- 根目录 `npm test`：134 项通过（含 #60 回归夹具清单良构 3 项、#61 前端 applyResplit/导出携带 3 项、#62 进度派生与四写入缝活动日 11 项；根测试递归含 `app/tests/`）。
- `cd app && node --test`：51 项通过。
- `cd app/src-tauri && cargo test`：171 项通过（含 `pdfparse_contract` 8 项、`pdfmap` 映射层内联 25 项与实测 fixture 6 篇、`pdfassets_contract` 5 项、`pdfparse_regression` 13 项——默认跑清单良构，全链 assert 模式本机实测 13/13 全绿；`schema_v4_contract` 5 项、`library_contract` 16 项、`migration_contract` 10 项）。
