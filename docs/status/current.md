...
- `npm test` 是统一测试入口，串起 `node --test`（自动发现 `tests/*.test.mjs`，当前 43 个测试）与 `tools/check_markdown.mjs`。`tools/check_parser.mjs` 没有断言、只输出 JSON 供人工查看，因此不在门禁内。
...
1. 浏览器端走查 `papers.js` 迁移：示例论文导入、精读生成、重切分作废提示与打卡显示。
