...
> 更新时间：2026-09-14（#75 模型轮遥测、解析计时与共享 HTTP 客户端已落地）
...
- 根目录 `npm test`：242 项通过（递归含 `app/tests/`）。
- `cd app && node --test`：159 项通过。
- `cd app/src-tauri && cargo test`：228 项通过（`pdfparse_regression` 默认跑清单良构，全链 assert 模式本机实测 13/13 全绿）。
...
| Issue | 内容 | 状态 |
| --- | --- | --- |
| [#75](https://github.com/attackingjensen/paper-30min/issues/75) | 模型轮遥测、解析计时探针与共享 HTTP 客户端 | 已完成（真实窗口走查未做） |
...
- #75 任务中心真实窗口尚未点验解析计时拆分与每轮遥测行（契约测试与呈现纯函数已覆盖）。
