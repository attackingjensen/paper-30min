...
本批最终代码在本机运行 `node --test` 213 项、语法检查 124 个文件、`npm run smoke` 14 项，`cargo test` 全套通过。另用 SQLite 在线备份取得现存 v6 书库的一致副本，仅对副本运行忽略标记的兼容性测试：迁移到 v7 后，论文、阅读位置和登记附件均可读取或校验；源库版本仍为 v6。窗口与安装版尚未走查。此清单仍需检查 UI 标记与样式、任务/侧车资源释放、构建打包和异常路径，新证实的问题按同一证据口径加入。
