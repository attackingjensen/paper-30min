...
项目源于实际自用需求，已有协作者参与工程化；现有材料没有要求发展为面向公众的多用户平台。分工约定：仓库持有者确认实际使用需求、产品取舍和新增能力是否必要；协作者负责缺陷修复、工程质量与交付方式，缺少实际使用反馈时不自行创造功能需求，涉及产品行为的决定在实施前与仓库持有者确认。局域网访问是无云服务器时的替代实现，不是必须长期保留的产品能力。来源：协作背景（Git 历史 35324e7）、[移动服务范围 #25](https://github.com/attackingjensen/paper-30min/issues/25)。
