...
export const PROTOCOL_PLACEHOLDERS = {
  ...
  'deep-dive': ['title', 'map', 'l2Summaries', 'sectionId', 'sectionTitle', 'sectionType', 'sectionText', 'sectionPages', 'sectionFocus'],
...
export const BUILTIN_PROTOCOL_PROMPTS = [
  {
    ...
    name: "建图 · 节薄摘要（L2）",
    stage: 'map-l2',
    description: "建图调用①：通读全文块模型，为每个原文章节生成薄摘要并打章节类型标",
    prompt: `<!-- Token 预算（#38 实测）：输入 ≈ 全文文本层（典型论文 2–6 万 token；全文超 ~20 万 token 时由调用方按节分片多次调用本提示词，再确定性合并）+ 关注点表数百 token；输出 ≈ 每节 1500 字量级 × 节数。任一调用装配后超 983,616 输入 token 由调用方报错拒绝，不截断。 -->

你是一位严谨的论文精读助手，正在为论文《{title}》执行建图第①步：通读全文块模型，为每个原文章节生成薄摘要（L2），并给出章节类型打标。
...
"""`,
  },
  {
    ...
    prompt: `<!-- Token 预算：常驻 = L1 地图 + 全部 L2（每节 1500 字量级）+ 当前节原文全送；当前节页图 ±1 页由调用方随消息附图（页图约 1902 token/页）。工具循环最多 12 步，工具结果只回窗口或指针。 -->
...
// ---------- 叠加规则与占位符装配 ----------
// 叠加规则（#55 决策 23）：协议提示词打底 + 该节类型关注点追加。map-l2 一次覆盖全部节，
// 叠加整表（生成某节摘要时按类型取用）；deep-dive 面向单节，叠加该节类型清单；map-l1 /
