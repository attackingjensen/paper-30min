// 技能库：运行时技能以 skills/*.md 为正式来源（经 initSkills 注入的 listSkillFiles 提供文件清单），
// BUILTIN_SKILLS 仅作来源失败时的降级兜底，两者内容由测试强制同步。
...
// loadOverrides() -> 覆盖映射对象、saveOverrides(obj)（接 settings 命令）。
...
/** 启动时注入技能来源并预热覆盖缓存；effectiveSkills/getSkill 保持同步语义。 */
export async function initSkills(impl = {}) {
  skillSource = { ...skillSource, ...impl };
  try {
    overridesCache = await skillSource.loadOverrides() || {};
  } catch (err) {
...
/** 启动时经注入缝拉取技能文件；任何失败都退回内置定义，不阻塞应用。返回实际生效的来源：'file' | 'builtin'。 */
export async function loadSkills() {
  try {
    const files = await skillSource.listSkillFiles();
