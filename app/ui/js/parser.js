...
let parserDeps = {
  // fetchText(url) -> string：net.fetch-text@1 的 chunk 累积结果，由 main.js 注入桥接实现。
  fetchText: async () => { throw new Error('parser.initParser 尚未注入 fetchText'); },
  // downloadPdf({arxivId, paperId}) -> AttachmentDto：files.download@1 等终态，由 main.js 注入。
  downloadPdf: async () => { throw new Error('parser.initParser 尚未注入 downloadPdf'); },
};

/** 注入 arXiv 网络能力；main.js 启动时接线，测试注入可控实现。 */
export function initParser(deps = {}) {
  parserDeps = { ...parserDeps, ...deps };
}

// pdf.js 惰性获取：首次使用时再读 window.pdfjsLib 并补 workerSrc。
function getPdfjs() {
  const lib = typeof window !== 'undefined' ? window.pdfjsLib : globalThis.pdfjsLib;
  if (!lib) throw new Error('pdf.js 尚未加载（window.pdfjsLib 为空）');
...
async function parsePdfBuffer(buf, fallbackName) {
  ...
  const title = (firstPageData && extractTitle(firstPageData)) || allLines[0]?.text || fallbackName || '未命名论文';
...
export async function parsePdfFile(file) {
  return parsePdfBuffer(await file.arrayBuffer(), file.name.replace(/\.pdf$/i, ''));
}

/** 内存中的 PDF 字节 → 章节切分（与 parsePdfFile 同管线；用于从附件存储读回的 PDF）。 */
export function parsePdfBytes(arrayBuffer, name = '') {
  return parsePdfBuffer(arrayBuffer, String(name).replace(/\.pdf$/i, ''));
}
...
/** 规范化 arXiv 编号：接受裸编号或 abs/html/pdf 链接，非法输入返回空串。 */
...
export async function fetchArxiv(id) {
  ...
  if (!normalized) throw new Error('请输入有效的 arXiv 编号或链接');
  const html = await parserDeps.fetchText(`https://arxiv.org/html/${normalized}`);
  return parseArxivHtml(html);
}

/**
 * 下载 arXiv PDF 到论文附件（id 固定 'pdf'）。经注入缝 downloadPdf（files.download@1 等终态），
 * 返回 AttachmentDto。需先有 paperId——先 createArxivPaper 建记录，再调本函数关联 PDF。
 */
export function downloadArxivPdf(arxivId, paperId) {
  return parserDeps.downloadPdf({ arxivId, paperId });
}
