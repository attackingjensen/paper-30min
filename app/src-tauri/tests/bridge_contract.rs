...
fn unknown_command_returns_unified_error() {
    ...
    let error = bridge::invoke(&registry, &library, "papers.delete@9", &json!({})).unwrap_err();
...
fn echo_round_trips_input_with_schema_version() {
    ...
    let result = bridge::invoke(&registry, &library, "bridge.echo@1", &input).unwrap();
...
fn app_info_reports_versioned_surface() {
    let (registry, library, _dir) = common::env();
    let result = bridge::invoke(&registry, &library, "app.info@1", &json!({})).unwrap();
    ...
    assert!(commands.contains(&json!("library.info@1")));
...
fn tasks_get_requires_task_id() {
    let (registry, library, _dir) = common::env();
    let error = bridge::invoke(&registry, &library, "tasks.get@1", &json!({})).unwrap_err();
...
fn tasks_get_unknown_id_returns_task_not_found() {
    let (registry, library, _dir) = common::env();
    let error = bridge::invoke(&registry, &library, "tasks.get@1", &json!({ "taskId": "task-999999" })).unwrap_err();
